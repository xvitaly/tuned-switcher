...
QString TunedManager::GetActiveProfile() const
{
    ...
    if (!DBusReply.isValid())
        qCWarning(LogManager::logdbus) << "Failed to get the active Tuned profile due to an error:" << DBusReply.error();
...
QStringList TunedManager::GetAvailableProfiles() const
{
    ...
    if (!DBusReply.isValid())
        qCWarning(LogManager::logdbus) << "Failed to get the available Tuned profiles due to an error:" << DBusReply.error();
    return DBusReply.value();
}
...
QTunedProfileMode TunedManager::GetProfileMode() const
{
    ...
    if (!DBusReply.isValid())
        qCWarning(LogManager::logdbus) << "Failed get current Tuned mode due to an error:" << DBusReply.error();
    return DBusReply.value();
}
...
QTunedProfileList TunedManager::GetAvailableProfiles2() const
{
    ...
    if (!DBusReply.isValid())
        qCWarning(LogManager::logdbus) << "Failed to get the available Tuned profiles with their descriptions due to an error:" << DBusReply.error();
    return DBusReply.value();
}
...
bool TunedManager::IsRunning() const
{
    ...
    if (!DBusReply.isValid())
        qCWarning(LogManager::logdbus) << "Failed to determine if the Tuned service is running due to an error:" << DBusReply.error();
    return DBusReply.value();
}
...
bool TunedManager::Start() const
{
    ...
    if (!DbusResult)
        qCWarning(LogManager::logdbus) << "Failed to start the Tuned service due to an error:" << DBusReply.errorMessage();
    return DbusResult;
}
...
bool TunedManager::Stop() const
{
    ...
    bool DbusResult = !(DBusReply.type() == QDBusMessage::ErrorMessage);
    if (!DbusResult)
        qCWarning(LogManager::logdbus) << "Failed to stop the Tuned service due to an error:" << DBusReply.errorMessage();
    return DbusResult;
}
